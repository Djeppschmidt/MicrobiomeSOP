...
if (length(fnFs) > 0) {
  ...
  if (file.exists(metadata_file)) {
    samdf <- read.csv(metadata_file)
    rownames(samdf)<-samdf$SampleID
    cat("Loaded metadata for", nrow(samdf), "samples\n")
  } else {
    print("Error: Metadata file not found. Add a file, or check YAML and directory for accuracy. Make sure the format is correct, and column labels are exact matches, including capitalization. The default file name is sample_metadata.csv \n")
  }
