...
if (file.exists("output/tables/deseq2_results.csv") && 
  ...
  create_butterfly_plot <- function(data, title) {
    ...
    # Prepare data for DESeq2 (left side, negative values)
    deseq2_data <- data.frame(
      TaxLabel = data$TaxLabel,
      Value = -abs(data$DESeq2_log2FC),  # Negative for left side
      Effect = data$DESeq2_effect,
      Method = "DESeq2"
    )
    
    # Prepare data for Quantitative (right side, positive values)
    quant_data <- data.frame(
      TaxLabel = data$TaxLabel,
      Value = abs(data$Quant_log2FC),  # Positive for right side
    ...
    p <- ggplot(plot_data, aes(x = TaxLabel, y = Value, fill = Effect)) +
      ...
      annotate("text", x = Inf, y = -max(abs(plot_data$Value), na.rm = TRUE) * 0.8, 
               label = "DESeq2", hjust = 0, vjust = 1, size = 4, fontface = "bold") +
      annotate("text", x = Inf, y = max(abs(plot_data$Value), na.rm = TRUE) * 0.8, 
               label = "Quantitative", hjust = 1, vjust = 1, size = 4, fontface = "bold")
