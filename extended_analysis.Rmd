...
if ("Group" %in% colnames(richness_data)) {
  ...
  if (length(unique(richness_data$Group)) > 2) {
    ...
    tukey_result <- TukeyHSD(residual_anova)
    print(tukey_result)
  }
}
...
This analysis uses DESeq2 to identify taxa with significantly different abundances between groups, accounting for sequencing depth and variability.
...
if (!requireNamespace("DESeq2", quietly = TRUE)) {
  ...
  if ("Group" %in% colnames(sample_data_df)) {
    ...
    # (required for samples with zeros)
    ...
    if (length(groups) >= 2) {
      ...
      if (nrow(sig_res) > 0) {
        ...
        if (nrow(sig_res) > 0) {
          ...
          tax_labels <- apply(tax_top, 1, function(x) {
            for (i in length(x):1) {
              if (!is.na(x[i]) && x[i] != "") {
          ...
          sample_groups <- sample_data_df[, c("Group")]
          names(sample_groups) <- rownames(sample_data_df)
