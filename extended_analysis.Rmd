...
if (any(config$metadata$treatment_column %in% colnames(alpha_div_merged))) {  # !! set group variable in config object
  library(tidyr)
  ...
  ggplot(alpha_long, aes(x = !!rlang::sym(config$metadata$treatment_column), y = Value, fill = !!rlang::sym(config$metadata$treatment_column))) +
    geom_boxplot() +
...
if (config$metadata$treatment_column %in% colnames(sample_data_df)) {
  
sample_data_df[[config$metadata$treatment_column]] <- as.factor(sample_data_df[[config$metadata$treatment_column]])
  
  # PERMANOVA test
  permanova_result <- adonis2(as.formula(paste0("dist_matrix ~ ", config$metadata$treatment_column, sep = "")),
                              data = sample_data_df,
                              permutations = 999)
  ...
  beta_disp <- betadisper(dist_matrix, sample_data_df[[config$metadata$treatment_column]])
...
if (config$metadata$treatment_column %in% colnames(sample_data_df)) {
  p <- p + stat_ellipse(aes(color = sample_data_df[[config$metadata$treatment_column]]), type = "t", level = 0.95)
}
...
if (!requireNamespace("DESeq2", quietly = TRUE)) {
  ...
  if (config$metadata$treatment_column %in% colnames(sample_data_df)) {
    ...
    diagdds <- phyloseq_to_deseq2(ps, 
                                  as.formula(paste0("~ ",
                                                 config$metadata$treatment_column,
                                                    sep = "")
                                             )
                                  )
    ...
    groups <- unique(sample_data_df[[config$metadata$treatment_column]])
    if (length(groups) >= 2) {
      contrast_name <- paste(groups[1], "vs", groups[2])
      cat("Contrast:", contrast_name, "\n\n")

      res <- results(diagdds, contrast = c(config$metadata$treatment_column, as.character(groups[1]), as.character(groups[2])))
      ...
      if (nrow(sig_res) > 0) {
        ...
        if (nrow(sig_res) > 0) {
          ...
          sample_groups <- setNames(sample_data_df[[config$metadata$treatment_column]], rownames(sample_data_df))
...
# Run Quantitative differential abundance

```{r quantitative-anova}
# Check if Q.ps exists
if (exists("Q.ps")) {
  # Get treatment column from config
  treatment_col <- config$metadata$treatment_column

  # Extract OTU table and sample data
  otu_mat <- as.matrix(otu_table(Q.ps))
  sample_df <- as.data.frame(as.matrix(sample_data(Q.ps)))
  sample_df[[treatment_col]] <- as.factor(sample_df[[treatment_col]])
  
  # Check if treatment column exists in metadata
  if (treatment_col %in% colnames(sample_df)) {
    # Get treatment groups
    treatment <- sample_df[[treatment_col]]
    ...
    if (length(unique(treatment)) >= 2) {
      ...
      # Apply ANOVA to each taxon
      for (i in 1:nrow(otu_mat)) {
        taxon_name <- rownames(otu_mat)[i]
        taxon_abundance <- otu_mat[i, ]

        # Create data frame for ANOVA
        anova_df <- data.frame(
          "abundance" = as.vector(taxon_abundance),
          "treatment" = treatment
        )

        # Run ANOVA
        anova_model <- aov(abundance ~ treatment, data = anova_df)
        anova_summary <- summary(anova_model)

        # Extract p-value
        p_value <- anova_summary[[1]]$`Pr(>F)`[1]

        # Calculate group means for effect direction
        group_means <- tapply(as.vector(taxon_abundance), treatment, mean)

        # Effect direction: which group has highest mean
        max_group <- names(which.max(group_means))
        min_group <- names(which.min(group_means))

        # Effect magnitude: difference in means (log2 fold change style)
        # Using the two most extreme groups
        max_mean <- max(group_means)
        min_mean <- min(group_means)

        # Calculate log2 fold change (add pseudocount to avoid log(0))
        pseudocount <- 1
        log2fc <- log2((max_mean + pseudocount) / (min_mean + pseudocount))
        ...
        anova_results[[i]] <- data.frame(
          ...
          "p_value" = p_value,
          "adjusted_p_value" = NA,  # Will be calculated later
      ...
      quantitative_anova_table <- do.call(rbind, anova_results)

      # Apply multiple testing correction
      quantitative_anova_table$adjusted_p_value <- p.adjust(quantitative_anova_table$p_value, method = "BH")

      # Add significance indicator
      quantitative_anova_table$significant <- quantitative_anova_table$adjusted_p_value < 0.05

      # Sort by p-value
      quantitative_anova_table <- quantitative_anova_table[order(quantitative_anova_table$p_value), ]
      ...
      write.csv(quantitative_anova_table,
                file.path(output_dir, "quantitative_anova_results.csv"),
                row.names = FALSE)
      cat(sprintf("Results saved to %s\n", file.path(output_dir, "quantitative_anova_results.csv")))

      # Print summary
      cat("\nQuantitative ANOVA Analysis Summary:\n")
      cat(sprintf("Total taxa tested: %d\n", nrow(quantitative_anova_table)))
      cat(sprintf("Significant taxa (adjusted p < 0.05): %d\n",
                  sum(quantitative_anova_table$significant, na.rm = TRUE)))
      cat("\nTop 10 most significant taxa:\n")
      print(head(quantitative_anova_table, 10))

    } else {
      cat("Need at least 2 groups for ANOVA analysis.\n")
    }
  } else {
    cat(sprintf("Treatment column '%s' not found in sample metadata.\n", treatment_col))
    cat("Available columns: ", paste(colnames(sample_df), collapse = ", "), "\n")
  }
} else {
  cat("Q.ps object not found. Make sure the quantitative phyloseq object was created successfully.\n")
}
```

# compare DESeq2 to Quantitative differential abundance

```{r compare-deseq2-quantitative}
# Check if both result tables exist
if (file.exists("output/tables/deseq2_results.csv") &&
    file.exists("output/tables/quantitative_anova_results.csv")) {

  # Load results
  deseq2_results <- read.csv("output/tables/deseq2_results.csv", stringsAsFactors = FALSE)
  quant_results <- read.csv("output/tables/quantitative_anova_results.csv", stringsAsFactors = FALSE)

  # Merge by taxon/ASV
  combined_results <- merge(
    deseq2_results[, c("ASV", "log2FoldChange", "padj", "baseMean")],
    quant_results[, c("Taxon", "log2_fold_change", "adjusted_p_value")],
    by.x = "ASV", by.y = "Taxon", all = FALSE
  )

  # Rename columns for clarity
  names(combined_results) <- c("ASV", "DESeq2_log2FC", "DESeq2_padj",
                                "baseMean", "Quant_log2FC", "Quant_padj")

  # Add taxonomy for labels
  tax_table_data <- as.data.frame(tax_table(ps))
  combined_results <- merge(combined_results, tax_table_data,
                           by.x = "ASV", by.y = "row.names", all.x = TRUE)

  # Create taxonomic label (use lowest available rank)
  combined_results$TaxLabel <- apply(combined_results[, c("Genus", "Family", "Order", "Class", "Phylum")],
                                     1, function(x) {
    for (i in 1:length(x)) {
      if (!is.na(x[i]) && x[i] != "") return(x[i])
    }
    return("Unknown")
  })
  ...
  combined_results$TotalAbundance <- rowSums(otu_mat)[combined_results$ASV]

  # Sort by total abundance
  combined_results <- combined_results[order(combined_results$TotalAbundance,
                                             decreasing = TRUE, na.last = TRUE), ]

  # Save combined results
  write.csv(combined_results, "output/tables/combined_deseq2_quantitative_results.csv",
            row.names = FALSE)
  cat("Combined results saved to output/tables/combined_deseq2_quantitative_results.csv\n")

  # Function to create butterfly plot
  create_butterfly_plot <- function(data, title) {
    # Determine significance and effect direction for both methods
    data$DESeq2_effect <- ifelse(is.na(data$DESeq2_padj) | data$DESeq2_padj >= 0.05, "no_effect",
                                 ifelse(data$DESeq2_log2FC > 0, "positive", "negative"))
    data$Quant_effect <- ifelse(is.na(data$Quant_padj) | data$Quant_padj >= 0.05, "no_effect",
                                ifelse(data$Quant_log2FC > 0, "positive", "negative"))

    # Prepare data for DESeq2 (left side - negate values to plot on left)
    deseq2_data <- data.frame(
      TaxLabel = data$TaxLabel,
      Value = -abs(data$DESeq2_log2FC),  # Negate to plot on left side
      Effect = data$DESeq2_effect,
      Method = "DESeq2"
    )

    # Prepare data for Quantitative (right side - keep positive)
    quant_data <- data.frame(
      TaxLabel = data$TaxLabel,
      Value = abs(data$Quant_log2FC),  # Keep positive to plot on right
      Effect = data$Quant_effect,
      Method = "Quantitative"
    )

    # Combine both datasets
    plot_data <- rbind(deseq2_data, quant_data)

    # Set factor levels for consistent ordering
    plot_data$TaxLabel <- factor(plot_data$TaxLabel, levels = unique(data$TaxLabel))

    # Define colors: blue=positive, red=negative, grey=no effect
    color_mapping <- c("positive" = "blue", "negative" = "red", "no_effect" = "grey")

    # Create butterfly plot
    p <- ggplot(plot_data, aes(x = TaxLabel, y = Value, fill = Effect)) +
      geom_bar(stat = "identity", width = 0.7) +
      scale_fill_manual(values = color_mapping,
                       labels = c("positive" = "Positive effect",
      ...
      annotate("text", y = min(plot_data$Value, na.rm = TRUE) * 0.8, x = nrow(data),
               label = "DESeq2", hjust = 0.5, vjust = -0.5, size = 5, fontface = "bold") +
      annotate("text", y = max(plot_data$Value, na.rm = TRUE) * 0.8, x = nrow(data),
               label = "Quantitative", hjust = 0.5, vjust = -0.5, size = 5, fontface = "bold")

    return(p)
  }

  # Create butterfly plot for 20 most abundant taxa
  if (nrow(combined_results) >= 20) {
    top_20_abundant <- combined_results[1:20, ]
    p_top20 <- create_butterfly_plot(top_20_abundant,
                                     "Comparison of DESeq2 vs Quantitative Analysis\n20 Most Abundant Taxa")
    print(p_top20)

    # Save plot
    ggsave("output/figures/butterfly_plot_top20_abundant.png", p_top20,
           width = 10, height = 8, dpi = 300)
  } else {
    cat("Not enough taxa for top 20 most abundant plot\n")
  }

  # Create butterfly plot for 20 least abundant taxa
  if (nrow(combined_results) >= 20) {
    bottom_20_abundant <- combined_results[(nrow(combined_results)-19):nrow(combined_results), ]
    p_bottom20 <- create_butterfly_plot(bottom_20_abundant,
                                       "Comparison of DESeq2 vs Quantitative Analysis\n20 Least Abundant Taxa")
    print(p_bottom20)

    # Save plot
    ggsave("output/figures/butterfly_plot_bottom20_abundant.png", p_bottom20,
           width = 10, height = 8, dpi = 300)
  } else {
    cat("Not enough taxa for bottom 20 least abundant plot\n")
  }

  # Summary statistics
  cat("\nSummary of Combined Analysis:\n")
  cat(sprintf("Total taxa compared: %d\n", nrow(combined_results)))
  cat(sprintf("Taxa significant in DESeq2: %d\n",
              sum(!is.na(combined_results$DESeq2_padj) & combined_results$DESeq2_padj < 0.05)))
  cat(sprintf("Taxa significant in Quantitative: %d\n",
              sum(!is.na(combined_results$Quant_padj) & combined_results$Quant_padj < 0.05)))
  cat(sprintf("Taxa significant in both: %d\n",
              sum(!is.na(combined_results$DESeq2_padj) & combined_results$DESeq2_padj < 0.05 &
                  !is.na(combined_results$Quant_padj) & combined_results$Quant_padj < 0.05)))

} else {
  cat("Missing required result files:\n")
  if (!file.exists("output/tables/deseq2_results.csv")) {
    cat("- output/tables/deseq2_results.csv not found\n")
  }
  if (!file.exists("output/tables/quantitative_anova_results.csv")) {
    cat("- output/tables/quantitative_anova_results.csv not found\n")
  }
  cat("Please run the DESeq2 and Quantitative differential abundance analyses first.\n")
}
```
...
# Export sample data
sample_data_export <- as.data.frame(as.matrix(sample_data(ps)))
