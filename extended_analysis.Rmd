...
if (exists("Q.ps")) {
  ...
  sample_df <- as.data.frame(sample_data(Q.ps))
  ...
  if (treatment_col %in% colnames(sample_df)) {
    ...
    if (length(unique(treatment)) >= 2) {
      ...
      for (i in 1:nrow(otu_mat)) {
        ...
        group_means <- tapply(taxon_abundance, treatment, mean)
...
if (file.exists("output/tables/deseq2_results.csv") &&
  ...
  create_butterfly_plot <- function(data, title) {
    ...
    deseq2_data <- data.frame(
      ...
      Value = -data$DESeq2_log2FC,  # Negate to plot on left side
    ...
    quant_data <- data.frame(
      TaxLabel = data$TaxLabel,
      Value = data$Quant_log2FC,  # Keep positive to plot on right
