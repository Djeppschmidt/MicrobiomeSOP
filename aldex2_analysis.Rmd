...
if (exists("ps")) {
  ...
  min_abundance <- 0.001  # 0.1% mean relative abundance
...
if (exists("aldex_results")) {
  ...
  summary_stats <- data.frame(
    Metric = c("Total taxa analyzed",
               "Significant (Welch BH < 0.05)",
               "Significant (Wilcoxon BH < 0.05)",
               "Large effect (|effect| > 1)",
               "Significant AND large effect",
               "Reference method",
               "Monte Carlo samples"),
    Value = c(nrow(aldex_results),
              sum(aldex_results$we.eBH < 0.05, na.rm = TRUE),
              sum(aldex_results$wi.eBH < 0.05, na.rm = TRUE),
              sum(abs(aldex_results$effect) > 1, na.rm = TRUE),
              sum(aldex_results$we.eBH < 0.05 & abs(aldex_results$effect) > 1, 
                  na.rm = TRUE),
